...
class BiEncoder(LightningModule):
    def __init__(self, hparams: Union[Namespace, dict]):
        ...
        self.query_encoder = BiEncoderModel.from_pretrained(self.hparams.base_pretrained_model, hparams=None)
        checkpoint = torch.load(hparams.load_path, map_location="cpu")
        self.query_encoder.load_state_dict(checkpoint["model_state_dict"], strict=False)
        self.passage_encoder = BiEncoderModel.from_pretrained(self.hparams.base_pretrained_model, hparams=None)
        if hasattr(hparams, "num_other_negatives"):
